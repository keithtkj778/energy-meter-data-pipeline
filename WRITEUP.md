...
This document explains the reasoning behind the technology, architecture, and
...
> NEM12 is the standard flat-file format used in the Australian energy market to
> exchange interval metering data. It's a fixed, line-oriented format where each
> line is a "record" identified by a numeric type (100 header, 200 NMI data
> details, 300 interval data, 900 end), and files can reach several gigabytes.
...
- **No CSV parser or ORM.** NEM12 is fixed-format with no quoting rules, so a
  ...
  actively hurt here: the goal is to emit raw, portable INSERT statements, not
...
Things I'd add next, in rough order by return on investment:
...
- Multi-row INSERTs are the simplest output format that still performs
  ...
  faster but requires a live connection, whereas the goal here is a portable,
  offline SQL artifact.
...
dominant convention in the Australian energy sector and matches how downstream
analytics (load curves, settlement calcs) typically read it. It's centralised in
`parseIntervalRecord` so a migration to a start-of-interval model is a one-line
change.
